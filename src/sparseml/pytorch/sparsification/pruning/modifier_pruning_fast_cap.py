...
@PyTorchModifierYAML()
class FastCAPruningModifier(OBSPruningModifier):
    """
    ...
    def _get_scorer(self, params: List[Parameter]) -> PruningParamsGradScorer:
        ...
        return FastCAPruningParamsScorer(
            params=params,
    ...
    def check_mask_update(
        ...
        # get applied sparsity since it is needed
    ...
    def _prepare(self,  module: Module):
        ...
        self.scorer.set_sparsity(np.mean(self._applied_sparsity))
...
class FastCAPruningParamsScorer(PruningParamsGradScorer):
    """
    ...
    def __init__(
        ...
        # init OBS handles
        self._handles: List[FastCAPHandle] = [None] * len(params)
        for i, param in enumerate(params):
            self._handles[i] = FastCAPHandle(
                param,
                block_size=block_size,
                damp=damp
            )

        # set defauly sparsity
        self._sparsity = None
    ...
    def _setup_FisherInverse(self, masks: List[Tensor]):
        ...
        for i, _ in enumerate(self._params):
            self._Finvs.append(
                FastEmpiricalBlockFisherInverse(
    ...
    def score_parameters(self) -> List[Tensor]:
        ...
        if self._is_main_proc:
            for i, handle in enumerate(self._handles):
                # set fisher inverse
                handle.set_F(self._Finvs[i].F)
                handle.set_sparsity(self._sparsity)
                # compute losses and weight traces
                handle.run()
                scores[i] = handle.weight.abs()
                # scores are losses
    ...
    def mask_update(self, masks: List[Tensor], mask_diffs: List[Tensor]):
        '''
        Set the weights from the chosen SPDY profile or determined by global scores.
        '''
        # get weights
        weights = [None] * len(self._params)

        if self._is_main_proc:
            for i, handle in enumerate(self._handles):
                weights[i] = handle.weight
                 # free handle
                handle.free()  

        self._broadcast_list_from_main(weights)
        # set weights got from the database
        for i, param in enumerate(self._params):
            param.data = weights[i].to(param.data.device)
...
class FastEmpiricalBlockFisherInverse:
    ...
    def add_grad(self, g: Tensor) -> None:
        """
