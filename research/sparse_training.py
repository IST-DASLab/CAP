...
group.add_argument('--gs-loader', action='store_true',
                    help='whether to create loader for GradSampler (default: False)')
group.add_argument('-gb', '--gs-batch-size', type=int, default=None, metavar='N',
...
def main():
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            sparsity_summary = json.loads(str(sparsity_info))

             # get current mean sparsity
            scheduled_sparsity = get_current_sparsity(manager, epoch)
            if model_ema is not None and not args.model_ema_force_cpu:
                if args.distributed and args.dist_bn in ('broadcast', 'reduce'):
                    utils.distribute_bn(model_ema, args.world_size, args.dist_bn == 'reduce')
                # do not evaluate EMA if in sparse stage
                if scheduled_sparsity == 0.0:
            ...
            if args.local_rank == 0:
                ...
                _logger.info(f'Scheduled sparsity: {scheduled_sparsity:.4f}')
